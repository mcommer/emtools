...
1. creation and modification of EM data files
2. creation and modification of EM model files and their corresponding grid (mesh) files
3. plotting of data and models

emtools is a derivative of Michael Commer's EMGeo toolboxes emgeo_prc, emgeo_modelcom, and em3d_dataplot. These tools were developed at Lawrence Berkeley Nat'l Lab, California. em3d_dataplot examples can for example be seen in:
*Commer,  M., and Newman, G.A., (2008). New advances in three-dimensional controlled-source electromagnetic inversion, Geophys. J. Int., 172/2, 513–535*. The **emtools_data_plot** extension for ModEM data files (1st version: 2023.10) was developed at Observatorio Nacional, Rio de Janeiro.
...
# Go to your home dir.
cd ~
# If not already present: create usr/local in your home
# Note: This is an installation in your home dir. You can also install into /usr/local,
#       then all steps need to be su-done.
mkdir -p usr/local
cd usr/local
# Clone the repository
git clone https://github.com/mcommer/emtools.git
# Run install.sh (note: needs csh, not bash)
csh -f emtools/install.sh
```

### 3. 🛒Export EMTOOLS shell variable
The EMTOOLS shell variable is needed by most tools of the **emtools** package and is generally set to
EMTOOLS = ${HOME}/usr/local  
It is recommended to set EMTOOLS at shell startup, for example for bash-users:
```shell
# this is in your ~/.bashrc file
export EMTOOLS=${HOME}/usr/local
# add the $EMTOOLS/bin/ directory to your executable-PATH
export PATH=${PATH}:${EMTOOLS}/bin
```
Alternatively, you can set EMTOOLS via the **emtools** configuration file "${HOME}/.emtools", which
is written after installation:
```shell
